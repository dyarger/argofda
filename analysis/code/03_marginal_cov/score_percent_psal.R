...
cv <- function(index, h_space) {
  ...
  df <- get_profile_data_subset(long = long, lat = lat, day = 45.25, h_time = 45.25, RG_def = FALSE,
                                h_space = h_space, mode = 'delayed',  min_prof=  5,
                                exclusion= TRUE)
  df$pressure <- round(as.numeric(df$pressure), digits = 2)

  if (!(lat %in% grid_pc_psal$lat[grid_pc_psal$long == long])) {
    b <- proc.time()
  ...
  temp_pc <- coefs_pc_psal[[which(grid_pc_psal$long == long &
                                    grid_pc_psal$lat == lat)]][[1]][,1:10]

  # get first few pc at this location
  # Calculate scores for each profile
  scores_5 <- unlist(get_scores(df, temp_pc[,1:5], K = 5, type = 'salinity',
                                knots = knots_pc)[[2]])
  scores_10 <- unlist(get_scores(df, temp_pc[,1:10], K = 10, type = 'salinity',
                                 knots = knots_pc)[[2]])
  per05 <- 1- mean(scores_5^2)/mean(df$salinity^2)
  per10 <- 1- mean(scores_10^2)/mean(df$salinity^2)
  cut_list <- list(cut(df$pressure, breaks = c(seq(0, 200, by = 10), seq(300, 2000, by = 100))))
  temp_aggr <- aggregate(df$salinity^2,cut_list, mean)[,2]
...
result <-mclapply(indexes, cv,
                  ...
                  h_space = h_space)
save(result, file = 'analysis/results/joint_TS_20/score_percent_psal.Rdata')

#file.to.save <- paste0('analysis/results/joint_TS_20/score_percent_psal.Rdata')
