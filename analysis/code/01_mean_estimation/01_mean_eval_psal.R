...
mean_est <- function(index, knots, penalty_mat, h_space) {
  ...
  df <- get_profile_data_subset(long = long, lat = lat, day = 45.25, RG_def = RG_def,
                                h_space = h_space, h_time = 45.25, mode = 'delayed', #months = 1:3,
  ...
  Phi =basis_eval,Psi = penalty_mat, W = W,Y = df$salinity)
  spline_results <- complete_optim(U = mod[['U']], V = mod[['V']], W = W,
                                   Omega = mod[['Omega']], Y = df$salinity,
                                   Phi_all = mod[['Phi_all']],
...
file.to.save <- paste0('analysis/results/mean_estimation/mean_one_stage_psal_', value, '.Rdata')
