...
source('analysis/code/08_mld/mld_source_comb.R') # load mld funs
...
pb_mld <- function(j, mean_files_temp, mean_files_psal, pressure_vec, K1, K2) {
  ...
  both_results <- lapply(1:nrow(df_indexes), function(x) {
    ...
    bs_results <- lapply(1:B, bs_fun, scores = scores, phi_deriv = phi_deriv, pred_pc = pred_pc, var_scores_half =
                           ...
                           delta = delta)
    mld_results <- do.call(rbind, (lapply(bs_results, function(x) x[[1]])))
    dens_results <- do.call(rbind, (lapply(bs_results, function(x) x[[2]])))
    return(list(mld_results, dens_results))
  })
